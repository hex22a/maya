...
maya.PluginManager.add('importcss', function(editor) {
	var self = this, each = maya.each;
			...
			each(rules, function(cssRule) {
				if (cssRule.styleSheet) {
					...
					each(cssRule.selectorText.split(','), function(selector) {
						selectors.push(maya.trim(selector));
					});
				}
			});
		}
	...
	function convertSelectorToFormat(selectorText) {
		...
		var inlineSelectorElements = maya.makeMap('a,img');
	...
	editor.on('renderFormatsMenu', function(e) {
		...
		var groups = [];
		maya.each(settings.importcss_groups, function(group) {
			group = maya.extend({}, group);
		...
		each(getSelectors(e.doc || editor.getDoc(), compileFilter(settings.importcss_file_filter)), function(selector) {
			if (selector.indexOf('.mce-') === -1) {
				if (!selectors[selector] && (!selectorFilter || selectorFilter(selector))) {
					...
					if (format) {
						var formatName = format.name || maya.DOM.uniqueId();
						...
						var menuItem = maya.extend({}, ctrl.settings.itemDefaults, {
							text: format.title,
							format: formatName
						});
