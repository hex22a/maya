...
maya.PluginManager.add('save', function(editor) {
	function save() {
		var formObj;

		formObj = maya.DOM.getParent(editor.id, 'form');
		...
		if (editor.getParam("save_onsavecallback")) {
			if (editor.execCallback('save_onsavecallback', editor)) {
				editor.startContent = maya.trim(editor.getContent({format: 'raw'}));
				editor.nodeChanged();
			}
	...
	function cancel() {
		var h = maya.trim(editor.startContent);
