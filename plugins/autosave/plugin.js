...
maya._beforeUnloadHandler = function() {
	var msg;

	maya.each(maya.editors, function(editor) {
...
maya.PluginManager.add('autosave', function(editor) {
	var settings = editor.settings, LocalStorage = maya.util.LocalStorage, prefix, started;

	prefix = settings.autosave_prefix || 'maya-autosave-{path}{query}-{id}-';
	...
	function isEmpty(html) {
		...
		html = maya.trim(typeof html == "undefined" ? editor.getBody().innerHTML : html);
	...
	window.onbeforeunload = maya._beforeUnloadHandler;
