...
maya.PluginManager.add('noneditable', function(editor) {
	var TreeWalker = maya.dom.TreeWalker;
		...
		function handleKey(e) {
			...
			function canDelete(backspace) {
				...
				function isNextPrevTreeNodeNonEditable() {
					...
					walker = new maya.dom.TreeWalker(container, editor.getBody());
					while ((node = (backspace ? walker.prev() : walker.next()))) {
						...
						if (node.nodeType === 3 && maya.trim(node.nodeValue).length > 0) {
							break;
						}
			...
			if (nonEditableParent && (keyCode < 112 || keyCode > 124) && keyCode != VK.DELETE && keyCode != VK.BACKSPACE) {
				...
				if ((maya.isMac ? e.metaKey : e.ctrlKey) && (keyCode == 67 || keyCode == 88 || keyCode == 86)) {
					return;
				}
	...
	function convertRegExpsToNonEditable(e) {
		var i = nonEditableRegExps.length, content = e.content, cls = maya.trim(nonEditClass);
	...
	editClass = " " + maya.trim(editor.getParam("noneditable_editable_class", "mceEditable")) + " ";
	nonEditClass = " " + maya.trim(editor.getParam("noneditable_noneditable_class", "mceNonEditable")) + " ";
