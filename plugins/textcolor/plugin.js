...
maya.PluginManager.add('textcolor', function(editor) {
	var cols, rows;
				...
				'<tr>' +
					'<td colspan="' + cols + '" class="mce-custom-color-btn">' +
						'<div id="' + id + '-c" class="mce-widget mce-btn mce-btn-small mce-btn-flat" ' +
							...
							'<button type="button" role="presentation" tabindex="-1">' + maya.translate('Custom...') + '</button>' +
						'</div>' +
					'</td>' +
				'</tr>'
			);
	...
	function onPanelClick(e) {
		...
		if (maya.DOM.getParent(e.target, '.mce-custom-color-btn')) {
			buttonCtrl.hidePanel();
			...
			editor.settings.color_picker_callback.call(editor, function(value) {
				...
				customColorCells = maya.map(tableElm.rows[tableElm.rows.length - 1].childNodes, function(elm) {
