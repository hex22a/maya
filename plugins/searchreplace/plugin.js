...
(function() {
	...
	function Plugin(editor) {
		...
		function showDialog() {
			...
			function notFoundAlert() {
				maya.ui.MessageBox.alert('Could not find the specified string.', function() {
					win.find('#find')[0].focus();
				});
			}

			var win = maya.ui.Factory.create({
				type: 'window',
				layout: "flex",
				pack: "center",
				align: "center",
		...
		function findSpansByIndex(index) {
			...
			nodes = maya.toArray(editor.getBody().getElementsByTagName('span'));
			if (nodes.length) {
		...
		self.replace = function(text, forward, all) {
			...
			node = editor.getBody();
			nodes = maya.toArray(node.getElementsByTagName('span'));
	...
	maya.PluginManager.add('searchreplace', Plugin);
})();
