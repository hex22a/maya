...
maya.PluginManager.add('lists', function(editor) {
	var self = this;
	...
	editor.on('init', function() {
		...
		function createNewTextBlock(contentNode, blockName) {
			...
			if (!editor.settings.forced_root_block) {
				...
				if (!hasContentNode && (!maya.Env.ie || maya.Env.ie > 10)) {
		...
		function getSelectedListItems() {
			return maya.grep(selection.getSelectedBlocks(), function(block) {
		...
		function splitList(ul, li, newBlock) {
			...
			function removeAndKeepBookmarks(targetNode) {
				maya.each(bookmarks, function(node) {
		...
		function normalizeList(element) {
			maya.each(maya.grep(dom.select('ol,ul', element)), function(ul) {
		...
		function applyList(listName) {
			...
			function getSelectedTextBlocks() {
				...
				maya.each(siblings, function(node) {
					if (isTextBlock(node)) {
					...
					if (maya.dom.BookmarkManager.isBookmarkNode(node)) {
			...
			maya.each(getSelectedTextBlocks(), function(block) {
		...
		function removeList() {
			...
			maya.each(getSelectedListItems(), function(li) {
				var node, rootList;
		...
		self.backspaceDelete = function(isForward) {
			function findNextCaretContainer(rng, isForward) {
				...
				walker = new maya.dom.TreeWalker(rng.startContainer);
		...
		editor.on('keydown', function(e) {
			...
			if (e.keyCode != 9 || maya.util.VK.metaKeyPressed(e)) {
				return;
			}
	...
	editor.on('keydown', function(e) {
		if (e.keyCode == maya.util.VK.BACKSPACE) {
			if (self.backspaceDelete()) {
				e.preventDefault();
			}
		} else if (e.keyCode == maya.util.VK.DELETE) {
