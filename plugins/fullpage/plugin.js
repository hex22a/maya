...
maya.PluginManager.add('fullpage', function(editor) {
	var each = maya.each, Node = maya.html.Node;
	var head, foot;
	...
	function showDialog() {
		...
		editor.windowManager.open({
			...
			onSubmit: function(e) {
				dataToHtml(maya.extend(data, e.data));
			}
		});
	}
	...
	function htmlToData() {
		...
		// Parse stylesheets
		data.stylesheets = [];
		maya.each(headerFragment.getAll('link'), function(link) {
	...
	function dataToHtml(data) {
		...
		maya.each(headerFragment.getAll('link'), function(stylesheet) {
		...
		// Add new
		maya.each(data.stylesheets, function(stylesheet) {
		...
		// Delete old
		maya.each(currentStyleSheetsMap, function(stylesheet) {
			stylesheet.remove();
		});
	...
	function parseHeader() {
		...
		return new maya.html.DomParser({
			validate: false,
			root_name: '#document'
		}).parse(head);
	}
	...
	function setContent(evt) {
		...
		var currentStyleSheetsMap = {};
		maya.each(headElm.getElementsByTagName('link'), function(stylesheet) {
	...
	function getContent(evt) {
		if (!evt.selection && (!evt.source_view || !editor.getParam('fullpage_hide_in_source_view'))) {
			evt.content = maya.trim(head) + '\n' + maya.trim(evt.content) + '\n' + maya.trim(foot);
		}
	}
