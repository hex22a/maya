...
maya.PluginManager.add('media', function(editor, url) {
	var urlPatterns = [
	...
	var embedChange = (maya.Env.ie && maya.Env.ie <= 8) ? 'onChange' : 'onInput';
		...
		var generalFormItems = [
			{
				...
				label: 'Source',
				onchange: function(e) {
					maya.each(e.meta, function(value, key) {
	...
	function dataToHtml(data) {
		...
		if (!data.source1) {
			maya.extend(data, htmlToData(data.embed));
			if (!data.source1) {
				return '';
			}
		}
		...
		maya.each(urlPatterns, function(pattern) {
			var match, i, url;
		...
		if (data.embed) {
			...
			maya.each(data, function(value, key) {
		...
		new maya.html.SaxParser({
			...
			start: function(name, attrs) {
				...
				if (name == "iframe" || name == "object" || name == "embed" || name == "video" || name == "audio") {
					...
					data = maya.extend(attrs.map, data);
				}
	...
	function sanitize(html) {
		...
		var writer = new maya.html.Writer();
		...
		}, new maya.html.Schema({})).parse(html);
	...
	editor.on('preInit', function() {
		...
		maya.each('video audio iframe object'.split(' '), function(name) {
		...
		maya.each('webkitallowfullscreen mozallowfullscreen allowfullscreen'.split(' '), function(name) {
			boolAttrs[name] = {};
		});
		...
		editor.parser.addNodeFilter('iframe,video,audio,object,embed,script', function(nodes, name) {
			...
			while (i--) {
				...
				placeHolder = new maya.html.Node('img', 1);
		...
		editor.serializer.addAttributeFilter('data-mce-object', function(nodes, name) {
			...
			while (i--) {
				...
				realElm = new maya.html.Node(realElmName, 1);
				...
				if (innerHtml) {
					innerNode = new maya.html.Node('#text', 3);
					innerNode.raw = true;
