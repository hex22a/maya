...
maya.PluginManager.add('textpattern', function(editor) {
	...
	function applyBlockFormat() {
		...
		if (textBlockElm) {
			walker = new maya.dom.TreeWalker(textBlockElm, textBlockElm);
			...
			if (firstTextNode) {
				...
				if (maya.trim(firstTextNode.data).length == pattern.start.length) {
					return;
				}
	...
	editor.on('keydown', function(e) {
		if (e.keyCode == 13 && !maya.util.VK.modifierPressed(e)) {
			handleEnter();
		}
	}, true);

	editor.on('keyup', function(e) {
		if (e.keyCode == 32 && !maya.util.VK.modifierPressed(e)) {
			handleSpace();
		}
	});
