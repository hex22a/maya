...
maya.ThemeManager.add('modern', function(editor) {
	var self = this, settings = editor.settings, Factory = maya.ui.Factory, each = maya.each, DOM = maya.DOM;
	...
	function createMenuButtons() {
		...
		function createMenu(context) {
			...
			removedMenuItems = maya.makeMap((settings.removed_menuitems || '').split(/[ ,]/));
	 ...
	 *
	 * @param {maya.ui.Panel} panel Panel to add focus to.
	 */
	...
	function renderInlineUI(args) {
		...
		editor.on('focus', function() {
			...
			if (args.skinUiCss) {
				maya.DOM.styleSheetLoader.load(args.skinUiCss, render, render);
			} else {
				render();
			}
		});
		...
		// Preload skin css
		if (args.skinUiCss) {
			maya.DOM.styleSheetLoader.load(args.skinUiCss);
		}
	...
	function renderIframeUI(args) {
		...
		if (args.skinUiCss) {
			maya.DOM.loadCSS(args.skinUiCss);
		}
		...
		if (settings.width) {
			maya.DOM.setStyle(panel.getEl(), 'width', settings.width);
		}
	...
	self.renderUI = function(args) {
		...
		if (skin) {
			...
			if (skinUrl) {
				...
				skinUrl = maya.baseURL + '/skins/' + skin;
			}

			// Load special skin for IE7
			// TODO: Remove this when we drop IE7 support
			if (maya.Env.documentMode <= 7) {
		...
		editor.on('ProgressState', function(e) {
			self.throbber = self.throbber || new maya.ui.Throbber(self.panel.getEl('body'));
